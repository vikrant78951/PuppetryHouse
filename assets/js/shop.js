...
jQuery(document).ready(function() {
	...
	jQuery( "#slider-range" ).slider({
		...
		slide: function( event, ui ) {
			jQuery( ".price-filter" ).text( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
		}
	});
	jQuery( ".price-filter" ).text( "$" + jQuery( "#slider-range" ).slider( "values", 0 ) +" - $" + jQuery( "#slider-range" ).slider( "values", 1 ) );	
	
	jQuery('.plus').click(function(){
		qty_field = jQuery(this).parent('.plus_minus').find('.qty');
